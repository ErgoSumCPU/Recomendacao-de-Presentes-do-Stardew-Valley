...
def carrega_dados():
    try:
        # Caminho para a pasta Álgebra a partir da pasta Python Co
        base_dir = os.path.dirname(os.path.abspath(__file__))  # Pasta do script
        parent_dir = os.path.dirname(base_dir)  # Pasta pai (Desktop)
        caminho = os.path.join(parent_dir, 'Álgebra', 'characters.csv')
        
        print(f"Procurando arquivo em: {caminho}")  # Debug
        
        if not os.path.exists(caminho):
            print("\nERRO: Arquivo characters.csv não encontrado!")
            print("Certifique-se que:")
            print("1. O arquivo characters.csv está na pasta Álgebra")
            print("2. A pasta Álgebra está no mesmo local que a pasta Python Co")
...
def mostra_resultados(resultados, presente):
    if resultados.empty:
        ...
        print("Dica: Verifique se digitou o nome corretamente em inglês (ex: 'Daffodil' para Narciso)")
        return
...
def main():
    ...
    if dados is None:
        ...
        print("Verifique se o arquivo characters.csv está na pasta Álgebra")
        return
